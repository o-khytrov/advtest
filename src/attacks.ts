...
import { Bounds } from './de/mutation';
import { Solver } from './de/solver';
    ...
    public static jsma(model, img, lbl, targetLbl, config: JsmaConfig) {
        ...
        for (let i = 0; i < Math.floor(ε / 2); i++) {
            ...
            tf.tidy(() => {  // (This must be in tf.tidy() as there are many intermediate NUM_PIXELS^2 matrices)
                ...
                let changedPixelsMask = tf.tensor(changedPixels);
                let [p, q] = Attacks.argmax2d(saliencyGrid.mul(changedPixelsMask));
